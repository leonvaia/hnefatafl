    ...
    impl GameState {
        ...
        pub fn generate_moves(&self, player: char) -> Vec<[usize; 4]> {
            ...
            for row in 0..7 {
                for column in 0..7 {
                    if self.board[row][column] == '.' { continue; }
                    if player == 'B' && self.board[row][column] != 'B' { continue; }
                    if player == 'W' && (self.board[row][column] != 'W' || self.board[row][column] != 'K') { continue; }

                    // four directions
                    let directions = [(-1,0),(1,0),(0,-1),(0,1)];
                    for (dr, dc) in directions {
                        let mut new_row = row as isize + dr;
                        let mut new_column = column as isize + dc;

                        while new_row >= 0 && new_row < 7 && new_column >= 0 && new_column < 7 {
                            if self.board[new_row as usize][new_column as usize] != '.' { break; }

                            let mv = [row, column, new_row as usize, new_column as usize];
                            if self.move_is_valid_for(&mv, player) {
                                moves.push(mv);
                            }
